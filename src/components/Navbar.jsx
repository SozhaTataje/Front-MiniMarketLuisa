      ...
      <nav className="p-5 max-w-screen-xl mx-auto flex justify-between items-center relative">
        ...
        <div className={`absolute md:static top-20 left-0 w-full md:w-auto bg-white md:bg-transparent z-50 transition-all duration-300 shadow-md md:shadow-none rounded-b-xl ${menuOpen ? "block" : "hidden"} md:flex md:items-center md:gap-6`}>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6 px-4 md:px-0 pb-4 md:pb-0 text-lg">
            ...
            <Link to="/productos" onClick={() => setMenuOpen(false)} className="hover:text-purple-600 font-medium">
              Productos
            </Link>
            <Link to="/sucursales" onClick={() => setMenuOpen(false)} className="hover:text-purple-600 font-medium">
              Sucursales
            </Link>

            {usuario?.roles?.includes("ROLE_ADMIN") && (
              <Link to="/admin/dashboard" onClick={() => setMenuOpen(false)} className="text-purple-600 font-bold">
                Admin Panel
              </Link>
            )}

            {!usuario ? (
              <Link to="/mi-cuenta" onClick={() => setMenuOpen(false)} className="hover:text-purple-600 font-medium">
                Mi cuenta
              </Link>
            ) : (
